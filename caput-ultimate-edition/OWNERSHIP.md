# CAPUT Sovereign Security Platform - Ownership & Attribution

## Copyright Notice

```
Copyright 2025 murray-ux
SPDX-License-Identifier: Apache-2.0
```

## Project Information

| Field | Value |
|-------|-------|
| **Project Name** | CAPUT Sovereign Security Platform |
| **Code Name** | GENESIS 2.0 |
| **License** | Apache License 2.0 |
| **SPDX Identifier** | Apache-2.0 |

---

## Founder & Lead Developer

### murray-ux

**Role:** Founder & Lead Developer

murray-ux is the creator and principal architect of the CAPUT Sovereign Security Platform. All original intellectual property, design patterns, and core innovations within this repository are the work of murray-ux.

**Contributions Include:**
- Original concept and system architecture
- GENESIS 2.0 sovereign security framework
- Biblical naming convention (MABUL, EBEN, RUACH, OHR, HADAAT, KERUV, NEPHESH)
- Pentagon 40 Rooms infrastructure design
- SHINOBI security layer architecture
- TETSUYA defense and risk management systems
- Evidence management and legal compliance frameworks
- AI integration patterns and Claude Code hooks

---

## Intellectual Property Declaration

All code, documentation, configurations, and creative works in this repository are:

1. **Originally Created By:** murray-ux
2. **Copyright Holder:** murray-ux
3. **Licensed Under:** Apache License 2.0

### Rights Retained

murray-ux retains:
- Full copyright ownership of all original works
- Right to relicense under different terms
- Right to create proprietary derivatives
- Attribution rights for all contributions

### Rights Granted (Apache 2.0)

Users of this software receive:
- Perpetual, worldwide, non-exclusive, royalty-free license
- Right to use, reproduce, and distribute
- Right to create derivative works
- Right to sublicense
- Patent license for contributions

---

## Attribution Requirements

When using, modifying, or distributing this software:

1. **Retain Copyright Notice:** Keep the copyright notice intact in all copies
2. **Include License:** Distribute the Apache 2.0 license with the software
3. **State Changes:** Document significant modifications
4. **Preserve NOTICE:** Include contents of NOTICE file if present

### Recommended Attribution Format

```
This software includes CAPUT Sovereign Security Platform
Copyright 2025 murray-ux
Licensed under Apache License 2.0
https://github.com/murray-ux/caput-sovereign-security
```

---

## Third-Party Components

This project incorporates third-party components under their respective licenses.
See [THIRD_PARTY_LICENSES](./licenses/THIRD_PARTY_LICENSES.md) for full details.

All third-party components are used in compliance with their respective licenses and are clearly attributed.

---

## Contact

For licensing inquiries, attribution questions, or permissions beyond the Apache 2.0 license scope:

**murray-ux**
- Project: CAPUT Sovereign Security Platform
- License: Apache-2.0

---

## Version History

| Version | Date | Description |
|---------|------|-------------|
| 1.0.0 | 2025-02-04 | Initial ownership documentation |

---

*This document establishes the legal ownership and attribution requirements for the CAPUT Sovereign Security Platform. All rights not explicitly granted under the Apache 2.0 license are reserved by murray-ux.*
